...
function validateTeamInputs(teamName: string, teamDescription: string) {
    if (!teamName || teamName.length > 255) {
        throw new Error("Team name must be provided and be less than 256 characters.");
    }
 ...
 */
export async function createTeam(teamName: string, teamDescription: string, userId: string) {
 ...
 * @throws Will throw an error if the Prisma query fails.
 */
export async function updateTeam(teamId: string, input: { teamName: string, teamDescription: string }) {
