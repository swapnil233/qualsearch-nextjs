...
export const createInvitations = async (invitations: ICreateInvitationsPayload[], invitedByUserId: string, teamId: string) => {
...
export const updateInvitationStatus = async (invitationId: string, status: InvitationStatus, invitedUserId?: string): Promise<Invitation> => {
    try {
        return prisma.invitation.update({
            where: { id: invitationId },
            data: {
                status: status,
                ...(invitedUserId && status === "ACCEPTED" && {
                    invitedUser: {
