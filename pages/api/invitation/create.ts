...
export default async function Handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (!isMember) {
            return res.status(403).send("You are not a member of this team, so you cannot invite others.");
        }

        // Verify the invitee email is not already a member of the team.
        const inviteeExists = team.users.some(user => user.email === invitedEmail);
        if (inviteeExists) {
            return res.status(400).send("The user you are trying to invite is already a member of the team");
        }
        ...
        if (inviteeAlreadyInvited) {
            return res.status(409).send("An invitation has already been sent to this email.");
        }
